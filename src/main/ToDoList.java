...
public class ToDoList {
	...
	public static void main(String[] args) throws IOException {
		...
		Controller.executeCMD("search 2103T"); //search 
	...
	private static void printout(ArrayList<Task> taskList) {
		for (int i = 0; i < taskList.size(); i++) {
			if (taskList.get(i) instanceof EventTask) {
				...
				System.out.println(et.getTaskName() + " from " + et.getStartTime() + " " + et.getStartDate() + " to " + et.getEndTime() + " "+ et.getEndDate());	
			}else if(taskList.get(i) instanceof DeadlineTask){
				DeadlineTask dl = (DeadlineTask) taskList.get(i);
				System.out.println(dl.getTaskName() + " by " + dl.getDeadlineTime() + " " + dl.getDeadlineDate());
